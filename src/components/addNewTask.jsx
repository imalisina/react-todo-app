...
const AddNewTask = () => {
    ...
    // Define a new state to store loading status
                ...
                <div className="relative">
                    <input disabled={isLoading} onChange={(event) => setNewTask(event.target.value)} type="text" className="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Add a new daily task" />
                    {
                        !isLoading
                            ...
                            : (<button disabled={isLoading} className="text-white absolute right-2.5 bottom-2.5 text-white bg-gradient-to-br from-pink-300 to-orange-200 font-medium rounded-lg text-sm px-8 py-2">
                                <svg aria-hidden="true" className="w-6 h-5 text-gray-100 animate-spin fill-black" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
